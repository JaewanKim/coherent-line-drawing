...
class dwLIC2
{
	...
	void initFlowField() {
		printf("initFlowField\n");
		if (m_nWidth && m_nHeight && !m_pFlowField)
			m_pFlowField = new float[m_nWidth * m_nHeight * 2];
		memset(m_pFlowField, 0.f, m_nWidth * m_nHeight * 2 * sizeof(float));
	}
	void initLICField() {
	...
	void initGaussianWeight(float sigma = 5.f) {
		if (m_nLength && !m_pGaussianWeight)
			m_pGaussianWeight = new float[m_nLength];
		memset(m_pGaussianWeight, 0.f, m_nLength * sizeof(float));
		getGaussianWeight(m_nLength, sigma);
	}
...
public:
	...
	dwLIC2(int nWidth, int nHeight, int nChannel, int nWidthStep, int nLength = 30, float fStepLength = 1.f, float sigma = 5.f)
		:m_nWidth(nWidth), m_nHeight(nHeight), m_nChannel(nChannel), m_nWidthStep(nWidthStep), m_nLength(nLength), m_fStepLength(fStepLength), m_fSigma(sigma) {
		initFlowField();
		initLICField();
		initNoiseField();
		initGaussianWeight(m_fSigma);
	}
	~dwLIC2() {
		if (m_pFlowField)
			delete[] m_pFlowField;
