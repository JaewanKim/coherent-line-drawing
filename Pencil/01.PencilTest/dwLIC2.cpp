...
//}
//
//void dwLIC2::setNoiseField(int x, int y, unsigned char v) {
//	if (x < 0 || x >(m_nWidth - 1) || y < 0 || y >(m_nHeight - 1)) {
//		std::cerr << "index error in setNoiseField" << std::endl;
//		return;
//	}
//	int index = m_nWidth * y + x;
//	m_pNoiseField[index] = v;
//
//	m_noiseList.push_back(dwNoise(x, y));
//}
//
//void dwLIC2::setNoiseField2(int x, int y, unsigned char r, unsigned char g, unsigned char b) {
//	if (x < 0 || x >(m_nWidth - 1) || y < 0 || y >(m_nHeight - 1)) {
//		std::cerr << "index error in setNoiseField" << std::endl;
//		return;
//	}
//	int index = m_nWidthStep * y + x * m_nChannel;
//	m_pNoiseField[index + 0] = b;
//	m_pNoiseField[index + 1] = g;
//	m_pNoiseField[index + 2] = r;
//
//	m_noiseList.push_back(dwNoise(x, y));
//}
//
...
////	//m_pNoiseField[index] = (range(rnd) > threshold) ? (unsigned char)32 : (unsigned char)224;
////	//m_pNoiseField[index] = (range(rnd) > threshold) ? (unsigned char)24 : (unsigned char)232;
////	//m_pLICField[index] = (range(rnd) > threshold) ? (unsigned char)0 : (unsigned char)255;
////
////	m_noiseList.push_back(dwNoise(x, y));
////}
...
//
//				if (m_nChannel == 3) {
//					totB += (m_pNoiseField[y * m_nWidthStep + x * m_nChannel + 0] * weight);
//					totG += (m_pNoiseField[y * m_nWidthStep + x * m_nChannel + 1] * weight);
//					totR += (m_pNoiseField[y * m_nWidthStep + x * m_nChannel + 2] * weight);
//				}
//				else
...
//				totR /= tot2;
//			}
//			m_pLICField[int(noise.y + 0.5f) * m_nWidthStep + int(noise.x + 0.5f) * m_nChannel + 0] = (unsigned char)totB;
//			m_pLICField[int(noise.y + 0.5f) * m_nWidthStep + int(noise.x + 0.5f) * m_nChannel + 1] = (unsigned char)totG;
//			m_pLICField[int(noise.y + 0.5f) * m_nWidthStep + int(noise.x + 0.5f) * m_nChannel + 2] = (unsigned char)totR;
//		}
//		else {
//			if (tot2 != 0.0)
//				tot /= tot2;
