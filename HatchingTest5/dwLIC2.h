...
class dwLIC2
{
	...
	std::vector <float*> m_pFlowField;

	std::vector<dwNoise> m_pCurve[2];
	float* m_pGaussianWeight = NULL;

	std::vector<dwNoise> m_noiseList;
	bool getFlowVectorInterpolated(float x, float y, float& dx, float& dy, int nFieldIndex);
	bool getFlowVectorRK4(float x, float y, float& dx, float& dy, int nFieldIndex);
	...
	void initFlowField() {
		for (int i = 0; i < m_nFieldNum; i++) {
			m_pFlowField.push_back(NULL);
			if (m_nWidth && m_nHeight && !m_pFlowField[i])
				m_pFlowField[i] = new float[m_nWidth * m_nHeight * 2];
			memset(m_pFlowField[i], 0, m_nWidth * m_nHeight * 2 * sizeof(float));
		}
...
public:
	dwLIC2() :dwLIC2(0, 0) {}
	dwLIC2(int nWidth, int nHeight, int nLength = 30, float fStepLength = 1.0f, int nFieldNum = 1, IplImage* m_pImg = NULL)
		:m_nWidth(nWidth), m_nHeight(nHeight), m_nLength(nLength), m_fStepLength(fStepLength), m_pIMG(m_pImg), m_nFieldNum(nFieldNum) {}
	~dwLIC2() {
		for (int i = 0; i < m_nFieldNum; i++)
		{
			if (m_pFlowField[i])
				delete[] m_pFlowField[i];
		}
	...
	void setFlowField(int x, int y, float dx, float dy, int nFieldIndex);
