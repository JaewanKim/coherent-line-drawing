...
int main(void) {
	...
	const char* path = "D:\\lab\\ETF\\Image\\bird.jpg";
	...
	printf("LIC \n");
	IplImage* pFlowImage;
	Mat ResultImg, GaussianImg, BilateralImg;
	...
	//GaussianImg = gaussian_blur(ResultImg, 1, 0.4f);
	//BilateralImg = bilateral_filter(ResultImg, original_image, 1, 0.5f, 100.f);

	Mat result = ResultImg;
	imwrite("D:\\lab\\ETF\\1.jpg", result);
	...
	for (int j = 0; j < height; j++) {
		for (int i = 0; i < width; i++) {
			...
			result.at<unsigned char>(j, i) = (unsigned char)(255 - value);
			//result.at<unsigned char>(j, i) = (unsigned char)(weight * 255);
		}
	}

	printf("FIN\n");
	imshow("result", result);
	imwrite("D:\\lab\\ETF\\paperTest_withDW.jpg", result);
	waitKey(0);
