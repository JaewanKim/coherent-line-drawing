...
void dwLIC2::setFlowField(int x, int y, float dx, float dy, int nFieldIndex) {
	...
	m_pFlowField[nFieldIndex][index] = dx;
	m_pFlowField[nFieldIndex][++index] = dy;
}
...
void dwLIC2::doLICForward(int weightflag) {
	...
	for (auto& noise : m_noiseList) {
		int nRandomField = range(rnd) % m_nFieldNum;
		...
		for (int i = -1; i < 2; i += 2, ++dir) {
			...
			x = ori_x;
			y = ori_y;
			if (!getFlowVectorInterpolated(x, y, ori_dx, ori_dy, nRandomField))
				continue;
			dx = (ori_dx * i);
			dy = (ori_dy * i);

			if (!getFlowVectorRK4(x, y, dx, dy, nRandomField))
				continue;
...
bool dwLIC2::getFlowVectorRK4(float x, float y, float& dx, float& dy, int nFieldIndex) {
	if ((x < 0) || (x > (m_nWidth - 1)) || (y < 0) || (y > (m_nHeight - 1)))
		return false;
	...
	getFlowVectorInterpolated(x + 0.5f * dx1, y + 0.5f * dy1, dx_temp, dy_temp, nFieldIndex);
	makeVectorCoherent(ori_dx, ori_dy, dx_temp, dy_temp);
	dx2 = m_fStepLength * dx_temp;
	dy2 = m_fStepLength * dy_temp;

	getFlowVectorInterpolated(x + 0.5f * dx2, y + 0.5f * dy2, dx_temp, dy_temp, nFieldIndex);
	makeVectorCoherent(ori_dx, ori_dy, dx_temp, dy_temp);
	dx3 = m_fStepLength * dx_temp;
	dy3 = m_fStepLength * dy_temp;

	getFlowVectorInterpolated(x + dx3, y + dy3, dx_temp, dy_temp, nFieldIndex);
	makeVectorCoherent(ori_dx, ori_dy, dx_temp, dy_temp);
...
bool dwLIC2::getFlowVectorInterpolated(float x, float y, float& dx, float& dy, int nFieldIndex) {
	if ((x < 0) || (x > (m_nWidth - 1)) || (y < 0) || (y > (m_nHeight - 1)))
		return false;
	...
	float dx1 = m_pFlowField[nFieldIndex][index];
	float dy1 = m_pFlowField[nFieldIndex][++index];
	float dx2 = m_pFlowField[nFieldIndex][++index];
	float dy2 = m_pFlowField[nFieldIndex][++index];

	index = (ny + 1) * m_nWidth * 2 + nx * 2;
	float dx3 = m_pFlowField[nFieldIndex][index];
	float dy3 = m_pFlowField[nFieldIndex][++index];
	float dx4 = m_pFlowField[nFieldIndex][++index];
	float dy4 = m_pFlowField[nFieldIndex][++index];
