...
int main(void) {
	...
	// Hatching Test with Different BlackNoise by Uniform Grid Intensity & Random field LIC

	const int length = 60;
	const float gauss_sigma = 1.0f;
	const int level = 2;
	dwLIC2 lic2(width, height, length, gauss_sigma, level);
	lic2.init();
	...
	for (int lev = 0; lev < level; lev++) {
		printf("ROT %d\n", lev * 70);
		if (lev != 0)
			rotate_field(refined_field, 70.f);

		for (int j = 0; j < height; j++)
			for (int i = 0; i < width; i++)
				rotated_field.at(lev).at<Vec2f>(j, i) = Vec2f(refined_field.at<Vec2f>(j, i)[0], refined_field.at<Vec2f>(j, i)[1]);
	...
	for (int l = 0; l < level; l++)
		for (int j = 0; j < height; j++)
			for (int i = 0; i < width; i++)
				lic2.setFlowField(i, j, rotated_field.at(l).at<Vec2f>(j, i)[0], rotated_field.at(l).at<Vec2f>(j, i)[1], l);
	...
	if (pFlowImage != NULL) {
		...
		imwrite("D:\\lab\\ETF\\ttest.jpg", MatImg);
		waitKey(0);
