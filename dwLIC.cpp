...
void dwLIC::MakeWhiteNoise()
{
	const int SIZE = n_xres * n_yres;
	pNoise = new unsigned char[SIZE];
	...
	for (int j = 0; j < n_yres; j++)
	{
		for (int i = 0; i < n_xres; i++)
		{
			...
			r = ((r & 0xff) + ((r & 0xff00) >> 8)) & 0xff;
			// pNoise[j * n_xres + i] = (unsigned char) r;
			pNoise[j * n_xres + i - 1] = (unsigned char)r;
		}
	}
}

void dwLIC::NoiseFromImage(Mat* img)
{
	const int SIZE = n_xres * n_yres;
	pNoise = new unsigned char[SIZE];
	for (int j = 0; j < n_yres; j++) {
		for (int i = 0; i < n_xres; i++) {
			for (int k = 0; k < img->Mat::channels(); k++) {
				pNoise[(n_yres - j - 1) * n_xres + i] = img->Mat::data[j * img->Mat::step1() + i * img->Mat::channels() + k];
			}
		}
	}
	bNoiseImage = true;
...
void dwLIC::NormalizVectrs()
{
	for (int j = 0; j < n_yres; j++)
	{
		...
		{
			int		index = (j * n_xres + i) << 1;
			float	vcMag = float(sqrt(pVectr[index] * pVectr[index] + pVectr[index + 1] * pVectr[index + 1]));
...
void dwLIC::FlowImagingLIC()
{
	...
	IplImage* iplimg = cvCreateImage(cvSize(n_xres, n_yres), 8, 1);
	cv::Mat matImg = cv::cvarrToMat(iplimg, false);
	cv::Mat* pOutputImg = &matImg;
		...
		for (int i = 0; i < n_xres; i++)
		{
			...
			for (advDir = 0; advDir < 2; advDir++)
			{
				...
				while (curLen < krnlen && advcts < ADVCTS)
				{
					...
					///obtain the texture value of the sample///
					const int pNoiseSize = (int)samp_y * n_xres + (int)samp_x;
					printf("%d, %d, %d \n", pNoiseSize, samp_y, samp_x);
					texVal = pNoise[pNoiseSize];
			...
			texVal = (texVal < 0.0f) ? 0.0f : texVal;
			texVal = (texVal > 255.0f) ? 255.0f : texVal;
			pOutputImg->Mat::data[(pOutputImg->rows - j - 1) * pOutputImg->Mat::step1() + i] = (unsigned char)texVal;
		}
	}
