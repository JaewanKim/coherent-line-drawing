...
void dwLIC::NoiseFromImage(Mat * img)
{
	...
	for(int  j = 0;   j < n_yres;  j ++)
	{
		for(int  i = 0;   i < n_xres;  i ++)
		{
			...
			{
				pNoise[(n_yres-j-1) * n_xres + i] = (unsigned char) img->Mat::data[j*img->Mat::step1() + i*img->Mat::channels + k];
			}
		}
	}
	bNoiseImage = true;
...
void dwLIC::FlowImagingLIC()
{
	...
	for(int  j = 0;	 j < n_yres;  j ++)
	{
		...
		{	
			...
			pOutputImg->Mat::data[(pOutputImg->Mat::row-j-1) * pOutputImg->Mat::step1() + i] = (unsigned char) texVal;
		}
	}
}
