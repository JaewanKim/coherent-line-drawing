...
int main(void) {
	...
	// Hatching Test with Gaussian Blurring & Bilateral Filtering & Alpha Blending
	const int length = 30;
	...
	for (int j = 0; j < height; j++)
		for (int i = 0; i < width; i++)
			//for (int k = 0; k < pImg->nChannels; k++)
				//lic2.m_pImageField[j * pImg->widthStep + i * pImg->nChannels + k] = pImg->imageData[j * pImg->widthStep + i * pImg->nChannels + k];
	...
	vector<Mat> GaussianImg, BilateralImg;
	for (int i = 0; i < level; i++)
	...
	for (int lev = 0; lev < level; lev++) {
		...
		lic2.doLICForward(1);	// NULL:Average, 0:Average, 1:Box, 2:Gaussian
		...
		string strFileName = "D:\\lab\\ETF\\paperTest_lev";
		string strFileExt = ".jpg";
		string strFullFileName = strFileName + to_string(lev) + strFileExt;
		imwrite(strFullFileName, MatImg);
		//GaussianImg.push_back(gaussian_blur(MatImg, 1, 0.4f));
		//GaussianImg.push_back(MatImg);
		BilateralImg.push_back(bilateral_filter(MatImg, original_image, 1, 0.4f, 100.f));
	}

	Mat img1 = BilateralImg.at(0);
	Mat img2 = BilateralImg.at(1);
	Mat result = alpha_blending(img1, img2, 50);

	printf("FIN\n");
	imshow("img1", img1);
	imshow("img2", img2);
	imshow("result", result);
	imwrite("D:\\lab\\ETF\\paperTest_img1.jpg", img1);
	imwrite("D:\\lab\\ETF\\paperTest_img2.jpg", img2);
	imwrite("D:\\lab\\ETF\\paperTest_result.jpg", result);
